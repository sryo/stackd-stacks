  ...
  for (const d of state.displays) {
    ...
    if (cfg.enableAnimations) {
      ...
      for (const t of targets) {
        ...
        // Cuts ~100 redundant setFrame round-trips/min generated by the
        // periodic display/spaces/windowsAll pushes when nothing changed.
        // "User is dragging" is handled separately by state.dragInFlight
        // (see events.js handleDragEnd) — never skip setFrame as a proxy
        // for that, because a weight-transfer retile produces a NEW
        // target that intentionally differs from the user's drag-end
        // frame, and we must apply it.
